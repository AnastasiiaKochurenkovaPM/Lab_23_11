...
let soldTickets_list = new Array();
...
class Ticket {
    ...
    constructor (place, vagon, passenger, train, id) {
        ...
        if (vagon === "" ||
            typeof vagon    === 'undefined') { this.vagon    = "Невідомий вогон"; }
        if (passenger === "" ||
...
function add_soldTicket (place, vagon, passenger, train, id) {

    let ticket = new Ticket(place, vagon, passenger, train, id);
    soldTickets_list.push(ticket);
...
function remove_ticket (id) {
    ...
    for (let z = 0; z < tickets_list.length; z++) {
        ...
        if (ticket.id === id) { soldTickets_list.push(ticket);
...
function remove_soldTicket (id) {

    for (let z = 0; z < soldTickets_list.length; z++) {

        let ticket = soldTickets_list[z];
        if (ticket.id === id) { soldTickets_list.splice(z, 1);
                                 return 1; }
...
function get_tickets_list (cured) {
    ...
    if (cured) { return soldTickets_list; }
...
function get_ticket_by_id (id, cured) {
    ...
    let list = cured ? soldTickets_list : tickets_list;
