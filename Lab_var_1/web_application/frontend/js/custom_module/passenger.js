...
class Passenger{
    ...
    constructor (name, number, train , id) {
        ...
        if (name === "" || typeof name === 'undefined') { this.name     = "Невідомий пасажир"; }
        if (number === "" || typeof number === 'undefined') { this.number      = "Не встановлено";  }
        if (train === "" || typeof train === 'undefined') { this.train= "Не встановлено";  }
        if (id === "" || typeof id === 'undefined') { this.id = ++last_passenger_id;  }
...
function set_passengers_list (data) {
    ...
    for (let element of data) {
        add_passenger(element.name, element.number, element.train, element.id);
    }
}
...
function print_passengers_list() {
    ...
    for (let z = 0; z <  passengers_list.length; z++) {
        ...
        console.log("\t" + "Поїзд: " +  passenger.train);
